...
export class ApplicationController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, type: ApplicationListItemResponseDto })
  ...
  async create(
    ...
    const application = await this.applicationService.create(vacancy.id, currentUser.id);
  ...
  @Get()
  @ApiOperation({ summary: 'Get paginated list of applications with vacancy details' })
  ...
  async listWithVacancies(
    ...
    const { data, totalCount } = await this.applicationService.listWithVacancies(
      currentUser.id,
      paginationDto,
    );
  ...
  async updateNotes(
    ...
    const application = await this.applicationService.updateNotes(id, dto.notes);
    return new ApplicationResponseDto(application);
  }
  ...
  @ApiParam({ name: 'id', description: 'Application ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Application deleted successfully' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Application not found' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized' })
